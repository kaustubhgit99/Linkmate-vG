...
-- =============================================
-- ENQUIRIES TABLE (required for contact form)
...
CREATE INDEX IF NOT EXISTS idx_enquiries_room  ON public.enquiries(room_id);
CREATE INDEX IF NOT EXISTS idx_enquiries_owner ON public.enquiries(owner_id);

ALTER TABLE public.enquiries ENABLE ROW LEVEL SECURITY;

-- Drop old enquiry policies first
DO $$ BEGIN
  DROP POLICY IF EXISTS "enq_insert" ON public.enquiries;
  DROP POLICY IF EXISTS "enq_select_owner" ON public.enquiries;
  DROP POLICY IF EXISTS "enq_select_sender" ON public.enquiries;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

-- Anyone (including anonymous) can submit an enquiry
CREATE POLICY "enq_insert" ON public.enquiries FOR INSERT WITH CHECK (true);
-- Owners can see enquiries for their rooms
CREATE POLICY "enq_select_owner" ON public.enquiries FOR SELECT USING (owner_id = auth.uid());
-- Senders can see their own enquiries
CREATE POLICY "enq_select_sender" ON public.enquiries FOR SELECT USING (sender_id = auth.uid());
